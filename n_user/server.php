...
if (isset($_POST['reg_user'])) {
  ...
  if (count($errors) == 0) {
  	...
  	$query = "INSERT INTO users (username, fname, lname, mobile, email, password) 
  			  VALUES('$username', '$fname', '$lname', '$mobile', '$email', '$password')";
  	mysqli_query($db, $query);
  	// Set the session variable to the inserted user id
...
if (isset($_POST['login_user'])) {
  ...
  if (count($errors) == 0) {
    ...
    if (mysqli_num_rows($results) == 1) {
      ...
      $_SESSION['success'] = "You are now logged in";
      echo $_SESSION['user_id']; // display the user ID
